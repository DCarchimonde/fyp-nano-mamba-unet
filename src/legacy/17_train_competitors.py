...
train_loader = DataLoader(Dataset(data=train_files, transform=train_transform), batch_size=2, shuffle=True)
val_loader = DataLoader(Dataset(data=val_files, transform=val_transform), batch_size=1, shuffle=False)


# ================= 3. 自动化炼丹核心函数 =================
def train_model(model, model_name, device, max_epochs=150):
    print(f"\n" + "=" * 50)
    print(f"🚀 开始集训: {model_name}")
    print("=" * 50)
    ...
    os.makedirs(r"D:\AI_FYP\models", exist_ok=True)
...
if __name__ == "__main__":
    ...
    # 选手1：Attention U-Net (经典 SOTA)
    att_unet = AttentionUnet(
        spatial_dims=3, in_channels=1, out_channels=4,
        channels=(16, 32, 64, 128, 256), strides=(2, 2, 2, 2)
    )
    train_model(att_unet, "Attention_UNet", device)

    del att_unet
    torch.cuda.empty_cache()

    # 选手2：移除 Mamba 的纯卷积 U-Net (消融实验)
    no_mamba = Ablation_NoMamba_UNet()
    train_model(no_mamba, "Ablation_NoMamba", device)

    print("\n🎉🎉🎉 诸神黄昏之战结束！所有对比模型权重已集齐！")