...
The experiment used PyTorch \cite{paszke2019pytorch} and MONAI on a single NVIDIA RTX 4060 GPU with 8GB VRAM. The random seed is fixed to 42 for Python, NumPy, and PyTorch, and CuDNN deterministic settings are enabled. All six result rows are accompanied by 40-case metric tables and 150-epoch training logs; their class averages and selected best epochs reproduce the aggregate result table. The full-cine run used Python 3.10.20, PyTorch 2.7.1 with CUDA 11.8, MONAI 1.5.2, the epoch-121 Nano-Mamba checkpoint, and batch size one. The patient split, preprocessing code, model definitions, evaluation code, full-cine result rows, figures, and hashes are retained in the project repository. Deterministic settings and sealed artifacts improve repeatability within a fixed software and hardware stack, but they do not guarantee bitwise identity across library versions or devices.
