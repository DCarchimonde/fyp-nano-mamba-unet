...
The No-Mamba ablation achieved 85.64\% mean DSC, which is 0.86 percentage points higher than Nano-Mamba U-Net. This is a particularly important negative result because it prevents over-claiming the Mamba-inspired module. The No-Mamba design is not parameter matched and uses 2.29M parameters compared with Nano-Mamba's 1.46M, so the comparison does not isolate sequence gating as the cause of the difference. It shows only that the executed No-Mamba architecture obtained higher Dice while Nano-Mamba retained the smaller reported parameter count.

The Half-Mamba variant achieved 84.95\% mean DSC, 0.17 percentage points above Nano-Mamba U-Net. In the absence of per-patient uncertainty or paired confidence intervals, this small difference should be reported descriptively and not interpreted as evidence that one variant is meaningfully better. A matched-capacity, multi-seed experiment would be required to study bottleneck size more reliably.
...
The audited summary records that the selected Nano-Mamba U-Net checkpoint was obtained at epoch 121 with a held-out validation mean DSC of 84.78\%. The full per-epoch training log was not present in the final-audit bundle, so this thesis does not reproduce intermediate learning-curve points that cannot be checked against the primary CSV. Likewise, the available legacy qualitative image could not be tied to both the rigorous checkpoint and a validation patient, so it is not presented as evidence. These omissions preserve a clear distinction between verified results and unsupported illustrations. The main empirical evidence remains Table \ref{tab:final_results}, generated from all 40 held-out validation cases.
...
\begin{enumerate}
    ...
    \item \textbf{Single split and seed:} Only seed 42 and one 80/20 patient split were reported. The evidence package did not contain per-case CSVs, so patient-level confidence intervals and paired model differences could not be calculated. Small score gaps should therefore not be treated as statistically established.
    ...
    \item \textbf{Metric convention and missing case evidence:} An empty prediction and empty reference for one class are assigned Dice 1 in the executed metric. The absent per-case outputs prevent checking whether this convention affected any aggregate value.
    ...
    \item \textbf{Limited metric and runtime provenance:} The main reported metric is Dice; no Hausdorff or average surface distance is available. The software/driver versions for the recorded FPS measurement were also not archived, so speed is an environment-specific engineering indicator rather than a universal benchmark.
\end{enumerate}

These limitations define the thesis boundary. The supplied aggregate files support an internally consistent validation summary and lightweight architectural exploration, but missing original run artefacts preclude complete end-to-end reproducibility or clinical deployment claims.
