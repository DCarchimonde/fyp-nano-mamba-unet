...
\begin{enumerate}
    ...
    \item \textbf{Temporal-fusion finding:} Fixed circular adjacent-frame probability fusion did not establish a resolved endpoint-Dice or EF improvement, but it reduced normalized LV-curve second difference for 19 of 20 patients with a paired 95\% bootstrap interval below zero. That pattern supports a specific finding---smoother curves---without implying that motion was learned.
    \item \textbf{Correction of thesis claims:} Unsupported statements about absolute state-of-the-art superiority, a complete Mamba implementation, learned temporal Mamba behaviour, dense cardiac motion tracking, or clinical readiness are explicitly excluded from the final thesis. A rigorous research project should report both positive and negative evidence accurately.
\end{enumerate}
...
This trade-off is central to the thesis. The project should not be judged only by whether it wins a single Dice ranking. Fewer trainable parameters can make a model worthwhile even when accuracy is acceptable rather than maximal. At the same time, parameter efficiency alone is not enough; the segmentation quality must remain clinically and technically reasonable. The final result suggests that Nano-Mamba U-Net sits in a useful middle ground: it is smaller than all evaluated models and more accurate than the basic 3D U-Net baseline, but it does not replace SegResNet16 as the accuracy leader.
...
Second, lightweight sequence-inspired modules remain worth exploring. Nano-Mamba U-Net achieved competitive validation performance with the lowest parameter count. This suggests that compact sequence processing at the bottleneck can be integrated into medical segmentation architectures without causing severe performance collapse. These findings warrant further investigation, although they do not establish superiority.

Third, the credibility of the comparison depends heavily on its validation design. The patient-level split yielded lower and more conservative estimates than the earlier exploratory evaluation, but those estimates are more trustworthy. In medical AI, an impressive Dice score is not informative if patient leakage or a loose evaluation protocol drives it. A properly validated lower score is preferable to an inflated result from a weak design.

Fourth, a spatial segmenter can provide complete-cycle measurements without becoming a dense motion tracker. Across all cine frames, the pipeline produced EF, phase, volume, centroid, and radial-distance trajectories; fixed fusion reduced curve roughness. However, the unresolved Dice and EF changes, together with missing manual masks at intermediate phases, require a separate validation boundary for every downstream output.

\section{Future Work}
Further work can improve the study in several directions.

\subsection{Evaluation on External and Official Test Sets}
Evaluation should next include the official ACDC test set or an independent external cardiac MRI dataset. Such testing would address generalization beyond the internal patient-level validation split. This matters particularly in medical imaging, where scanner vendor, acquisition protocol, image resolution, and patient population may each alter model performance.
...
The present study already processes complete 4D cine sequences and derives global segmentation-based trajectories, but the trained network remains spatial and the temporal fusion is fixed. An extension could instead take several phases jointly and learn temporal representations with an explicit consistency objective. Such a model should be compared against the current frame-wise and fixed-fusion baselines, rather than assuming that a learned temporal block is automatically better.
